...
from realtime import listen_for_user_realtime
from run_agent import run_agent, as_tool, Agent, conclude, current_agent
from speech import listen_for_user, speak_text
...
def talk_to_user(message: str):
    ...
    speak_text(message)
    return listen_for_user()


@toolbox.tool
async def talk_to_user_realtime(message: str):
    """
    Use this function to communicate with the user using realtime audio input.
    All communication to and from the user **MUST**
    be through this tool.
    :param message: The message to send to the user.
    :return: The user's spoken response transcribed to text.
    """
    _agent = current_agent.get()
    name = _agent['name'] if _agent else 'Agent'
    print(f'{name}: {message}')
    speak_text(message)
    return await listen_for_user_realtime()
...
async def main(agent_config: Path, message: str):
    ...
    if not message:
        message = 'Please begin the conversation with the user.'
