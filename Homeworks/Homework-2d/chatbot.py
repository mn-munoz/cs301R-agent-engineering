...
class ChatAgent:
    ...
    async def get_response(self, user_message: str):
        ...
        response = await self._ai.responses.create(
            ...
            tools=[eigen_tool, next_devotional_tool, find_lds_quote_tool], # type: ignore 
        )
    ...
    async def check_for_tool_call(self, response):
        ...
        for tool_call in response.output:
            ...
            if tool_call.name == "get_eigenvalues":
                tool_result = get_eigenvalues(**args)
            elif tool_call.name == "get_next_devotional":
                tool_result = get_next_devotional()
            elif tool_call.name == "find_lds_quote":
                tool_result = find_lds_quote(**args)
            else:
                continue

            print(f"Tool call: {tool_call.name} with args {args} produced output {tool_result}")
            tool_outputs.append({
