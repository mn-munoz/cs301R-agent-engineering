        ...
        while True:
            ...
            for item in response.output:
                if item.type == 'reasoning':
                    ...
                    yield 'reasoning', f'Loaded MCP tools from {item.server_label}'

                elif item.type == 'mcp_call':
                    if item.error:
                        yield 'reasoning', f'{item.server_label}.{item.name} error: {item.error}'
                    else:
                        yield 'reasoning', f'{item.server_label}.{item.name}({item.arguments})'
